...
| **LandParcel** | id, generatorId FK, parcelName, ecosystemType, state, district, village, geofence, totalAreaHa, claimedCredits, contractAddress, tokenId, status, totalCreditsIssued, createdAt, updatedAt | cuid pk, strings, floats, timestamps | `[x]` | Geofence stored as JSON string for SQLite; includes State, District, Village administrative location |
