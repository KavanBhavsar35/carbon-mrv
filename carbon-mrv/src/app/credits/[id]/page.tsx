      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs font-mono text-neutral-400">
          <div className="flex items-center gap-2">
            ...
            <span>/</span>
            <span className="text-neutral-200">Credit #{credit.creditId}</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-neutral-400">Token Standard:</span>
            <span className="text-emerald-400 font-mono font-semibold">ERC-1155 Semi-Fungible</span>
          </div>
        </div>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-[#0c1410] border border-emerald-950/70 rounded-2xl p-6 sm:p-8 shadow-xl">
            <div className="space-y-6 relative before:absolute before:inset-0 before:left-5 before:w-0.5 before:bg-emerald-900/60 before:h-full">
              {provenanceSteps.map((step, idx) => {
                const IconComponent = step.icon;
                return (
                  <div key={idx} className="relative flex items-start gap-4 group">
                    {/* Circle Node */}
                    <div className="w-10 h-10 rounded-2xl bg-emerald-950/80 border border-emerald-700/60 flex items-center justify-center text-emerald-300 shrink-0 z-10 shadow-md group-hover:scale-105 group-hover:border-emerald-500 transition-all">
                      <IconComponent className="w-5 h-5" />
                    </div>

                    {/* Step Card Content */}
                    <div className="flex-1 p-4 rounded-xl bg-neutral-950/50 border border-emerald-950/40 hover:border-emerald-800/60 transition-all">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Link
                            href={step.href}
                            className="text-neutral-400 hover:text-emerald-300 transition p-1"
                            title="Inspect stage"
                          >
        ...
        <section id="history" className="space-y-4">
          ...
          <div className="bg-[#0c1410] border border-emerald-950/70 rounded-2xl p-6 shadow-xl space-y-4">
            {credit.history.map((event, index) => (
              <div
                key={index}
                className="p-4 rounded-xl bg-neutral-950/60 border border-emerald-950/40 space-y-2 hover:border-emerald-800/40 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono text-neutral-400 pt-1">
                  <div>
                    ...
                    <span className="truncate block text-neutral-300">{event.from}</span>
                  </div>
                  <div>
                    <span className="text-neutral-500 uppercase text-[10px] block">To:</span>
                    <span className="truncate block text-neutral-300">{event.to}</span>
                  </div>
                </div>

                <div className="pt-2 border-t border-neutral-900 flex flex-wrap items-center justify-between gap-2 text-xs font-mono">
                  <div className="flex items-center gap-2">
                  ...
                  {event.certificateId && (
                    <Link
                      ...
                      className="text-emerald-400 hover:text-emerald-300 underline text-xs font-semibold inline-flex items-center gap-1"
                    >
        ...
        <div className="rounded-2xl bg-gradient-to-r from-emerald-950/80 via-[#0c1410] to-[#0c1410] border border-emerald-800/50 p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
          <div>
            ...
            <p className="text-xs text-neutral-300 mt-1 max-w-xl">
              25.000 tCO₂e was permanently retired by EcoTech Global Holdings for Scope 1 &amp; 2 ESG compliance.
